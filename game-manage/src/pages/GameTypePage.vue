  ...
  <div class="table">
    ...
    >
      <el-form
        ...
        class="demo-ruleForm"
      >
        <el-form-item label="游戏分类名称" prop="type" size="mini">
          <el-input
    ...
    >
      <el-form :rules="UpdateRules" ref="form" :model="form" label-width="60px">
        <el-form-item label="游戏分类名称" size="mini" prop="type">
  ...
  methods: {
    ...
    addPeople() {
      ...
      this.$refs["registerForm"].validate(valid => {
        if (valid) {
          HttpManager.setGameType(this.registerForm)
            .then(res => {
              if (res.code === 0) {
    ...
    saveEdit() {
      this.$refs["form"].validate(valid => {
        if (valid) {
          HttpManager.updateGameType(this.form)
            .then(res => {
              if (res.code === 0) {
                this.getData();
                this.$notify.success({
    ...
    // 确定删除
    deleteRow() {
      HttpManager.deleteGameType(this.idx)
        .then(res => {
          if (res.code === 0) {
            this.getData();
    ...
    delAll() {
      ...
      if (this.multipleSelection.length == 0)
        ...
        this.$confirm("确定要删除选中的文件吗?", "提示")
          .then(() => {
            HttpManager.removeBatchOfGameType(ids).then(res => {
              if (res.code === 0) {
                this.getData();
                this.notify("删除成功", "success");
              } else {
    ...
    exportToExcel() {
      ...
      require.ensure([], () => {
        ...
        const tHeader = ["游戏分类Id", "游戏分类名称"];
        const filterVal = ["id", "type"];
        const list = this.multipleSelection;
        const data = this.formatJson(filterVal, list);
        export_json_to_excel(tHeader, data, "游戏分类信息导出excel");
      });
    },
